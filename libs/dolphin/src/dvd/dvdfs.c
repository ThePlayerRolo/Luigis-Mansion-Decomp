...
BOOL DVDReadAsyncPrio(DVDFileInfo* fileInfo, void* addr, s32 length, s32 offset,
        ...
        if (!((0 <= offset) && (offset < fileInfo->length))) {
            OSPanic(__FILE__, 735, "DVDReadAsync(): specified area is out of the file  ");
        }
        
        if (!((0 <= offset + length) && (offset + length < fileInfo->length + DVD_MIN_TRANSFER_SIZE))) {
            OSPanic(__FILE__, 741, "DVDReadAsync(): specified area is out of the file  ");
        }
...
long DVDReadPrio(struct DVDFileInfo * fileInfo, void * addr, long length, long offset, long prio) {
    ...
    if (!((0 <= offset) && (offset < fileInfo->length))) {
        OSPanic(__FILE__, 805, "DVDRead(): specified area is out of the file  ");
    }

    if (!((0 <= offset + length) && (offset + length < fileInfo->length + DVD_MIN_TRANSFER_SIZE))) {
        OSPanic(__FILE__, 811, "DVDRead(): specified area is out of the file  ");
    }
